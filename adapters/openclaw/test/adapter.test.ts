...
describe("registration", () => {
  ...
  it("keeps the reader registered but refuses execution when reader.enabled is false", async () => {
    const dir = workspace();
    const path = join(dir, "ws", "conf.txt");
    writeFileSync(path, "max_retries = 3\n");
    const { api, registered, modelCalls } = configured(dir, { reader: { enabled: false } });
    const shunt = new ContextShuntPlugin(api);
    shunt.register();

    expect(registered.tools).toContain(READER_TOOL_NAME);
    const out = JSON.parse(
      await shunt.onReaderTool({ question: "What is it?", paths: [path] }, { sessionKey: "s1" }),
    );
...
describe("reader tool", () => {
  ...
  it("delegates an empty provider to host routing and reports a custom selection", async () => {
    const dir = workspace();
    const path = join(dir, "ws", "conf.txt");
    writeFileSync(path, "max_retries = 3\n");
    const attempted: string[] = [];
    const { api } = configured(dir, {
      reader: { model: "custom-reader-v2", provider: "" },
    });
    api.runtime.llm.complete = async (opts: any) => {
    ...
    expect(attempted).toEqual(["custom-reader-v2"]);
    expect(out.code).toBe("ANSWERED");
    expect(out.provenance.requested_provider).toBeUndefined();
    expect(out.provenance.requested_model).toBe("custom-reader-v2");
    expect(out.provenance.resolved_provider).toBe("acme-ai");
    expect(out.provenance.resolved_model).toBe("custom-reader-v2");
    expect(out.provenance.attribution_status).toBe("resolved");
  });
