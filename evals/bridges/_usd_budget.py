...
The ledger charges the conservative upper bound before a physical provider request.
Reservations are never refunded: a timeout, crash, lost response, or retry can still have
been billed.  SQLite's ``BEGIN IMMEDIATE`` makes the cumulative check and insertion one
transaction across concurrent and resumed evaluator processes.
...
class UsdBudgetLedger:
    ...
    def _metadata(self) -> dict[str, str]:
        return {
            ...
            "reservation_policy": "pre_dispatch_upper_bound_never_refunded_v1",
        }
    ...
    def reserve(
        ...
        cost = (
            Decimal(input_token_upper_bound) * self.pricing.max_input_rate
            + Decimal(max_output_tokens) * self.pricing.max_output_rate
        ) / TOKENS_PER_MILLION
        nano = int((cost * NANO_USD_PER_USD).to_integral_value(rounding=ROUND_CEILING))
        if nano <= 0:
        ...
        with closing(self._connect()) as connection:
            ...
            used = int(
                connection.execute(
                    "SELECT COALESCE(SUM(reserved_nano_usd), 0) FROM reservations"
                ).fetchone()[0]
            )
            if used + nano > limit:
                connection.rollback()
    ...
    def summary(self) -> dict[str, Any]:
        with closing(self._connect()) as connection:
            reserved, count = connection.execute(
                "SELECT COALESCE(SUM(reserved_nano_usd), 0), COUNT(*) FROM reservations"
            ).fetchone()
            statuses = {
                row[0]: int(row[1])
        ...
        return {
            ...
            "reserved_usd": format(Decimal(reserved) / NANO_USD_PER_USD, "f"),
            "remaining_usd": format(Decimal(limit - reserved) / NANO_USD_PER_USD, "f"),
