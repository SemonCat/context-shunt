...
def main() -> None:
    ...
    if not os.environ.get("CONTEXT_SHUNT_LUNA_BUDGET_DB"):
        raise SystemExit(
            "NOT_RUN: set CONTEXT_SHUNT_LUNA_BUDGET_DB to the durable USD 10.00 ledger"
        )
    ...
    try:
        ...
        if usd_budget.get("limit_usd") != "10" or not Decimal("0") <= reserved_usd <= Decimal("10"):
            errors.append("cumulative USD 10.00 ceiling was not enforced")
