...
def _setup(tmp_path):
    ...
    body = {"data": {"result": [{"stream": {"job": "synthetic"}, "values": [
        [str(index), json.dumps(row, separators=(",", ":"))] for index, row in enumerate(logs)
    ]}]}}
    (tmp_path / "ws").mkdir()
    path = tmp_path / "ws" / "loki.json"
    path.write_text(json.dumps(body, separators=(",", ":")))
    session = ShuntSession(
        "sess", make_config(tmp_path), make_capability(),
...
def test_counts_groups_and_distinct_fields_in_minified_loki_json(tmp_path):
    ...
    env = session.inspect(_request(entry, {
        "kind": "aggregate", "records_pointer": "/data/result",
        "expand_pointer": "/values", "record_pointer": "/1", "parse_json": True,
    ...
    assert result["distinct"][0] == {
        "path": "/trace_id", "count": 4, "values": ["tr-a", "tr-c", "tr-d", None],
        "values_complete": True,
    }
    assert result["groups"] == [
        {"key": ["billing"], "count": 2}, {"key": ["checkout"], "count": 1},
        {"key": [None], "count": 1}, {"key": [{"missing": True}], "count": 1},
    ]
