...
def _request(entry, request_id, **overrides):
    request = {
        "schema_version": "1.3", "request_id": request_id, "operation": "read",
        "question": "What is retry_limit?",
        "sources": [{
            "source_id": entry.source_id, "snapshot_id": entry.snapshot.snapshot_id,
...
def test_reuses_complete_exact_query_and_reports_zero_usage_for_hit(tmp_path):
    ...
    stats = session.stats({
        "schema_version": "1.2", "request_id": "req_stats", "operation": "stats",
        "page_size": 8,
    })
    row = next(
        record for record in stats["stats"]["records"]
...
def test_query_selector_budget_model_snapshot_and_authorization_boundaries_do_not_hit(tmp_path):
    ...
    changed = session.read(_request(
        entry, "req_snapshot",
        sources=[{
            "source_id": entry.source_id, "snapshot_id": "sha256:" + "0" * 64,
            "selector": {
                "kind": "lines", "start": 1, "end": entry.snapshot.line_count,
            },
        }],
    ))
...
def test_least_recently_used_answer_is_evicted_after_32_entries(tmp_path):
    ...
    for index in range(33):
        session.read(_request(
            entry, f"req_{index}", question=f"What is retry_limit? variant {index}"
        ))
