 ...
 *
 * For the optional oversized-tool-result capture mode (`tool_result_capture`), the report
 * must identify the host seam's actual visibility and replacement guarantees. A middleware
 * seam may support embedded or host-owned dynamic results while its ingress sanitizer still
 * imposes a cap; content at that boundary is partial or refused, never silently described as
 * a complete capture. Observe-only result paths remain unsupported for replacement. See
 * `docs/capability-matrix.md` for the file-and-line evidence behind each reason.
 */
