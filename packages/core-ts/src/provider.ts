...
export const READER_SYSTEM_PROMPT = [
  ...
  "3. State every factual claim as a separate object in `claims`: `text` is the assertion in your own words, with no citation marker such as \"[c1]\" written into it - the caller renders markers from `citation_ids` mechanically, so a marker you place by hand is never trusted. Every claim must name the exact source key or identifier that carries the answer, even when the question paraphrases it. Copy identifiers (including hyphenated or compound names), numbers, and boolean or yes/no values exactly as they appear in the excerpt rather than paraphrasing them: if the source says `attempt_limit = 7`, write `attempt_limit is 7`, never `the attempt limit is 7`; if it says `service_contact: atlas-ops`, write `service_contact is atlas-ops`, never `atlas-ops is the contact`. Keep the source key as the grammatical subject even when that sounds less natural. When the answer selects a record by a condition, keep both bindings in the same claim: for `{\"eligible\":false,\"route_alias\":\"nova-east\"}`, write `route_alias is nova-east because eligible is false`, never only `route_alias is nova-east`. Paraphrase everything else freely. `citation_ids` lists every entry in your `citations` array that supports that claim; a claim with no citation_ids is dropped, so never state a fact without one.",
  ...
  "6. When the excerpt addresses lines, each physical line is shown as `N: ` followed by its text, including blank lines - for example `42: attempt_limit = 7`. That `N: ` gutter is not source text: never include its digits, colon or the space after it in a quote, and always set `line_start`/`line_end` to the exact gutter number(s) of the lines you cite. Read the gutter; never count lines yourself.",
