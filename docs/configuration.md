...
| `tool_result_capture.enabled` | boolean | `false` | Requests the optional oversized-tool-result capture path. OpenClaw 2026.9.3 uses the official middleware, subject to read-only eligibility, ingress ceilings and atomic reducer cutover. On Hermes, additionally requires `tool_result_capture.host_ordering_verified_locally: true` to be reported supported at all, but attested ordering alone does not prove the caller can consume a pointer. Pointer delivery also requires an invocation-scoped consumer-capability descriptor; the currently inspected host does not supply one, so the adapter produces bounded no-handle legacy compaction. See [`capability-matrix.md`](capability-matrix.md#tool_result_capture-on-hermes-021-what-changed-and-what-did-not). The deprecated key `suma_post_tool.enabled` is still accepted as an alias; setting both to disagreeing values is refused with `TOOL_RESULT_CAPTURE_CONFIG_CONFLICT`. |
