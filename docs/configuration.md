...
| `tool_result_capture.enabled` | boolean | `false` | Requests the optional oversized-tool-result capture path. OpenClaw automatic capture remains retired. On Hermes, both operator attestations below are required before the hook registers. Pointer delivery additionally requires the immutable invocation descriptor on every call; absent/incomplete scope produces bounded no-handle legacy compaction. Unmodified Hermes 0.21.3 lacks that delivery seam; see the exact proposal and probe in [`capability-matrix.md`](capability-matrix.md#tool_result_capture-on-hermes-021-what-changed-and-what-did-not). The deprecated key `suma_post_tool.enabled` remains an alias; conflicting sections are refused with `TOOL_RESULT_CAPTURE_CONFIG_CONFLICT`. |
...
| `tool_result_capture.host_consumer_scope_verified_locally` | boolean | `false` | An explicit **operator attestation** that the installed Hermes host forwards a fresh immutable direct/deferred consumer descriptor for each invocation. The exact-image probe can verify a candidate host; this flag is never inferred or set by the adapter. Ordering attestation alone cannot register capture. |
